...
> **状态：已完成（归档，2026-09-25）**。Phase 11~12 都已实现并推送。本文只作历史记录：以后的改动不再往这里追加修订记录，独立审计也**不**以本文为参考，以当前代码、提交信息与 `AGENTS.md` 为准。
...
>
> **R45（两个密码明文显示，用户要求；细节见 `pair-plan.md` 的 R45）** 覆盖本文 R34 / R36 / R37 里「保存会清空输入框」「两个密码各有『保存（清空输入框，只进系统凭据库）』」「连接即落盘……清空输入框」的表述：两个密码现在和服务器地址一样一直显示在框里（新增 `pair_get_secret` / `pair_get_server_password` 读回明文），保存与「立即连接」后都不清空，框里改过才落盘。值仍只存在系统凭据库。
...
「界面与文档」三个，之后 R37 的三轮复审又收了两个小提交：`b1faebd`（部署侧的文案与注释）
与 `1d01f9b`（界面只在配对密码真的换了时才说「已替换」）。以上提交都已推到
