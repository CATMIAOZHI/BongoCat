...
> - **打字期间不算猫的活动**：输入框聚焦 → `pair.runtime.localInputPaused`（本窗口瞬时状态，不落盘）→ `usePairState` 停发宠物快照并先发一帧空快照让对方把猫放下，`useDevice` 不把这期间的键盘/鼠标按键喂给本机贴图。没有这一步，打字会被全局键鼠监听当成按键：自己的猫乱按，对方还看得见你按了什么。鼠标**位置**仍然照发（对方的猫继续跟着指针看）。
...
> - **语音按钮复用现成的按住说话**：录音实体是猫咪窗口那一份 `usePairVoiceRecorder`，点一下开始、再点一下发送（快捷键按住说话照旧）。
>
> **平台事实（已核实，2026-09-23）**
>
