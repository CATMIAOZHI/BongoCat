...
> **状态：已完成（归档，2026-09-25）**。Phase 1~6 与其后的修订（到 R46）都已实现并推送。本文只作历史记录：以后的改动不再往这里追加修订记录，独立审计也**不**以本文为参考，以当前代码、提交信息与 `AGENTS.md` 为准。
...
>
> **后续计划另开一份**：Phase 7~9（自建云服务器中继 / P2P / 60Hz）见 `docs/pair-plan-cloud-p2p.md`，其修订记录 R20~R25；与本文冲突时以那份为准。本文的 Phase 1~6 已实现并推送，章节内容保持原样。
>
...
>
> **R1 后续修正（2026-09-25，卡顿现场取证）** 取消上述 16ms 置顶保持线程及状态表，改为开关变化时调用一次 Tauri `window.set_always_on_top`，由 Windows 保持各窗口的置顶属性。ETW 中三个最忙线程占主进程 CPU 样本约 84%，这些线程有栈样本的 99% 以上位于 `NtUserSetWindowPos`，表明持续重排窗口存在显著开销。各窗口仍可独立开启/关闭置顶，但不再持续抢占其他置顶窗口的前方；销毁接口保留为空实现。§5 的线程管理方案仅作历史记录。实际输入卡顿是否完全消失，仍需更换修复版本后复测。
>
...
>
> **R39（§29 / §30 猫咪窗口上的聊天浮层）** 用户要求把气泡和输入框直接做在**猫咪窗口**上：窗口顶上加一条浮层（`src/components/chat-overlay/index.vue`），上面是最新几条气泡、下面是常驻输入条（**麦克风 + 输入框 + 发送**，按用户要求不放附件按钮）。落地口径：
>
> - **窗口变高，猫不缩**：浮层高度 = 模型高 × `CHAT_OVERLAY_RATIO`（`src/constants/index.ts`，0.62），窗口总高 = 模型高 × (1 + 比例)；猫咪本体贴底、按模型比例占一块，所以猫的大小和位置不变（`pages/main/index.vue` 里那层 wrapper）。**只有开启双人联机时**才多出这一条，单机时窗口尺寸与过去完全一样。
> - **镜像只作用在猫咪那一层**：`-scale-x-100` 从窗口根节点移到猫咪 wrapper 上，否则会把浮层里的文字一起镜像。
> - **浮层里最多 3 条气泡**（`OVERLAY_BUBBLE_MAX`）：猫咪窗口顶上是窄带，按聊天窗口默认的 5 条会顶到上沿被裁掉。条数取「同时显示的气泡数」与 3 的较小值。
> - **在浮层里打字也算猫的活动**（用户要求，R39 初稿是先停发、落地前按用户口径翻过来）：全局键鼠监听照旧把这段输入喂给本机贴图与对方，所以自己的猫和对方的猫都会跟着按。**不再有任何「聚焦输入框就暂停同步」的机制**（初稿那套 `pair.runtime.localInputPaused` 已整个删掉，别再加回来）。
> - **悬停淡出让开浮层那一条**：`hideOnHover` 的判定里把窗口顶上 `CHAT_OVERLAY_RATIO / (1 + CHAT_OVERLAY_RATIO)` 那块算作「不算在窗口里」，否则鼠标一进窗口就整窗变透明，输入框根本点不到。
> - **穿透仍然是整窗设置**：`passThrough` 开着时整个窗口都点不到（包含浮层），要用输入框就得先关掉穿透。Tauri 只能按窗口设置 `setIgnoreCursorEvents`，做不到只让浮层可点。
> - **语音按钮复用现成的按住说话**：录音实体是猫咪窗口那一份 `usePairVoiceRecorder`，点一下开始、再点一下发送（快捷键按住说话照旧）。**（R41 起改成：点一下开始、再点一下转入「待确认」，要再点一次「发送」才发出去。）**
>
> **R40（设置页与错误提示，用户逐条提出）** 都是界面层的口径修正：
>
> - **服务器地址改成「草稿 + 保存」**：以前输入框直接 `v-model` 绑在 `settings.relay.url` 上，每敲一个字符就写一次设置并落盘——粘贴长地址或用输入法时会丢字符、光标乱跳。现在和两个密码一样：输入框里是草稿（`relayUrlInput`），点「保存」（或直接点「立即连接」）才写进设置；没保存时下面挂一条「地址改过了，点保存才会生效」。落盘值到得比组件晚（`@tauri-store/pinia` 是异步载入的），所以用一个 `watch`，只在「草稿还等于上一个值」（用户没动过）时回填。
> - **服务器密码 / 配对密码改成明文输入**（用户要求）：`Input.Password` 改成 `Input`，并给服务器密码补上「复制」——保存之后输入框会清空，那是唯一能把它发给对方的时机。凭据库里的明文仍然是只写不读，界面上永远拿不回来。
> - **「输入统计」那一行排版修好**：内容是 `w-full` 的 Flex，把 `ProListItem` 左侧的标题与说明挤成了一列竖排字（用户给的截图）。改成 `vertical` 布局、数字排在标题下面，累计一行用次要色。
> - **「最近一次错误」给详情**：除了那句结论，再列出**这次连接用的地址**、**服务器密码填了没有**、**配对密码填了没有**——403 那一类错误里用户最需要的就是这三条。Rust 侧的 403 文案也拆成两种（`client.rs`：**没带**服务器密码 → 「这台服务器要求填…」；**带了却被拒** → 「服务器密码不对…」），不再用一句「不正确或还没填」把两件事糊在一起。
> - 顺手把 `shareInputStats` 的说明从「默认关闭」改成「默认开启」，跟上 R38。
>
> **R41（§44 / §45 语音的二次确认，用户要求）** 录完**先不发**，用户确认了才发：
>
> - Rust 侧拆成三段：`pair_stop_recording` 从「结束并发送」改成「结束并**收下**」——wav 落在临时目录里，返回 `VoiceDraft { path, durationMs }`；新增的 `pair_send_recording` 才真正走附件管线（`stage = true`，成功时把临时 wav 收进附件缓存）；`pair_cancel_recording` 现在管两种取消（正在录的那一段 / 录完待确认的那一份），另一个清理入口是重新开始录音（`pair_start_recording` 先把上一份草稿连文件一起删掉），保证临时目录里不留孤儿 wav。
> - 前端：`usePairVoiceRecorder` 多一份 `pending`（外加 `playing` / `sending`），`release()` 只是转成待确认；猫咪窗口下方那条提示在待确认时变成「试听 / 发送 / 取消」（试听用 `<audio>` + asset protocol 播本机的 wav，不走网络、不问对方）。**对方不在线时「发送」是灰的、点了也不发起**——发送失败路径会删掉临时文件，等于白录一段，所以宁可先不让点。
> - 快捷键按住说话与浮层上的麦克风按钮走的是同一条状态机：松开同样只到「待确认」。
>
> **R42（§29 - §32 聊天窗口的观感，用户要求）** 用户给的截图里聊天窗口是半透明的、语音消息用的是系统原生播放器：
>
> - **窗口改成不透明**：根节点 `bg-black/45` → `bg-[#191c22]`（外加一圈 `ring-white/10`）。窗口配置仍然是 `transparent: true`——圆角正是靠它才成立，底色换成实色之后桌面就透不进来了。
> - **语音消息自绘播放条**：`<audio controls>` 换成 播放键 + 进度条 + 时长，走 `new Audio()` + asset 协议（新 composable `usePairVoicePlayback`）；时长从音频元数据读，读到之前显示 `--:--`，文件还没落到本机时按钮禁用。
> - **其余是排版**：头部状态点与分隔线、气泡圆角内距、滚动条压细、空状态图标、附件操作改成小胶囊、输入区变成盒子 + 圆形发送键（与 R39 浮层同款）。拖窗口与 Enter / 输入法那套键位一个字没动。
> - 没有做拖拽上传：现在只有粘贴图片与纸夹选文件两条路，HTML5 drop 要先关掉 Tauri 的 `dragDropEnabled`，属于新功能、不在本次范围。
>
> **R43（重绘与两个猫咪窗口的拖动逻辑，用户发现）** 用户先说「对方猫拖大小的逻辑和自己猫一样」、随即更正为「不一样」，并指出「刚刚聊天的修改把重绘的逻辑改坏了」。两句都成立，这是**两个 bug**：
>
> - **自己猫：R39 漏了一步**。R39 把 `debouncedResize` 从 `useModel().handleResize()` 换成自己算尺寸时，把 `handleResize` 里那句 `live2d.resizeModel(modelSize)` 一起丢了——pixi 的 `resizeTo: window` 只改画布尺寸、不动模型，所以 R39 之后拖自己猫的窗口**猫不再重绘**（猫停在旧缩放上，背景与按键贴图相对猫错位）。修法：在 `debouncedResize` 里补回 `live2d.resizeModel(modelSize.value)`，并把 `resizing` 的清除挪进 `finally`。
> - **对方猫：从来没有「按比例摆正」**（这条不是 R39 弄坏的，`applySize()` 从 `082d2d9` 起就没变过）。它拖边缘是自由拉伸：比例不写回、`object-cover` 的贴图跟等比居中的猫必然错位、下一次重算尺寸（改设置、换模型、重启）又跳回去。修法：照自己猫那套三段式，真相源仍然是 `remoteCat.scale`，**不能**写 `catStore.window.scale`（R11）。
> - 有意保留的差异：两侧缩放数值各自独立；对方猫没有聊天浮层，不引入 R39 的 `extraRatio`。仍未做的可选项：对方猫的 Shift+右键缩放、右键比例菜单、`keepInScreen` / `hideOnHover`。
>
> **R44（R40~R43 的实现细节修正，独立审计提出）** R40~R43 落地后跑了两个只读审计（代码 / UX），按它们的清单修完之后又有一轮复审，复审里剩下的问题在本条收口。**R44 覆盖 R41 / R42 / R43 中与下面冲突的表述**：
>
> - **R41 那句「`pair_start_recording` 先把上一份草稿连文件一起删掉」改成「麦克风真的打开之后才删」**（`mod.rs`：`discard_draft()` 挪到 `recorder.start()?` 之后）。理由是「重录时麦克风打不开」这条路径：删在前面会把用户上一段还没确认的录音连文件一起弄丢。前端同步：`usePairVoice.press()` 只在 `started` 为真时才清 `pending`。代价是这段时间 `pending` 与 `recording` 同时为真，所以猫咪窗口的待确认胶囊要写成 `v-if="recordingPending && !recording"`——两个胶囊不能同时在。
> - **R42 那句「读到之前显示 `--:--`」收窄成「只有正在播的那一条」**：`usePairVoicePlayback` 的 `labelOf` 对非当前条返回空串，`--:--` 只出现在当前这条读元数据的那几十毫秒里；没落到本机的语音按钮 `disabled` + `opacity-40`，文案 `pages.chat.player.waiting`。
> - **R43 的反推口径固化为「物理像素」**：两侧 `debouncedResize` 都用 `(await appWindow.size()).width` 反推缩放，**不用 CSS 的 `innerWidth`**。`size()` 走的是 `Webview.size()`（webview 客户区的物理像素），与 `setSize(PhysicalSize)` 是同一个量的两种读法，任何系统缩放下都相等——所以「拖一次最多两轮」就收敛，不会每 100ms 反复 `setSize`。这同时修掉了「换模型（`useModel.handleResize` 一直用物理宽度）与拖窗口在非 100% 缩放下互相打架」的旧问题。
> - 「失败标签串条」修掉：播放失败的状态从全局 `failed` 改成 `failedId`（对外 `isFailed(id)`），只有真正读不出来的那一条会显示失败文案。
> - 设置页两处小补：地址「复制」改成复制**框里显示的那一串**（框空时退回已保存值，按钮在「框空且从没保存过」时才禁用）；明文提醒的比对对象从「已保存地址」改成**输入框里的地址**（草稿为空才退回已保存值），免得 R40 之后要等到保存才提醒。
> - 语音相关的临时 `voice-*.wav` 不再留孤儿：启动时（`mod.rs` 的 `setup`）扫一遍临时目录删掉上次运行留下的，运行中待确认的那一份不动。
> - **P3 也一并收掉了**（第二轮收口）：①`press()` 的 `.then` 不再拿 `round` 比——按下之后马上松开或取消时 `round` 已经变了、可 Rust 那边草稿确实已作废，留着就是一条点不开的死待确认；判定只看「start 成没成」，丢掉上一段时给一句 `recordingDiscarded`（键盘与麦克风两条路都覆盖）。②语音时长按 `messageId` 缓存（`durations`），换条播放后上一条的时长不再消失；没播放过的条仍然不显示时间。③待确认胶囊不再把 `enabled` 与 `peerOnline` 合成一句，发送中显示 `sendingRecording`（细分口径见下面那条）。④「明文提醒」保持只看 `runtime.plaintext`（Rust 的 `is_plaintext_endpoint` 是唯一真相，前端不重写一份同样的判定），所以它是「连过之后」提醒而不是「粘上就提醒」——这是有意保留的。
> - **第二轮 UI / UX 审计又提了一批 P1/P2，也在本条收口**（这轮改动量最大的是第一条，它是**渲染 bug**）：
> - **`bg-white/NN` / `color-white/NN` / `border-white/NN` / `ring-white/NN` 这一整套在三个叶子窗口里根本不生效**。UnoCSS 的 `presetAntd` 把主题里的 `white` 收成 `var(--ant-color-white)` 并**丢掉 `/NN`**；而 `--ant-color-*` 是 antd 组件在运行时通过 cssinjs 挂上去的，猫咪窗口 / 聊天窗口 / 对方猫窗口里一个 antd 组件都没有，静态 CSS 里也从来没有它的定义。所以「禁用态 / 悬停态 / 收到的气泡底色 / 失败态是红的」这些差别全都不存在。同类被牵连的还有 `color-red-3`（`var(--ant-red-3)`）、`rounded-lg` / `rounded-sm`（`var(--ant-border-radius-*)`）、`text-sm` / `text-lg` / `text-xl`（`var(--ant-font-size-*)`）。修法：这四处改成显式颜色/尺寸（`bg-[#ffffff1a]`、`text-[#ff7875]`、`rounded-[0.5rem]` 这种），改完用仓库自己的 `uno.config.ts` 生成 CSS 逐条核对过。（**设置页不受影响**：它在 antd 组件里，那儿的 antd token 类照旧用。）
> - **联机状态在叶子窗口里不再糊成一句「对方离线」**。新增 `pairStateKey(connection, enabled)`（`stores/pair.ts`，单测在 `src/stores/pair.spec.ts`），把「联机没打开 / 正在连接 / 还没连上 / 连不上 / 对方不在线」分开；浮层、聊天窗口、对方猫三处共用 `pages.pairState.*`。设置页的 `status.peerOffline` 同时改成「对方没上线」，免得同一个词两处含义不同。
> - **浮层与聊天窗口的键位/提示对齐**：浮层的 Enter 补上 `isComposing` / `keyCode 229` 兜底（中文输入法选词的 Enter 以前会直接把拼音发出去）；「发不出去的原因」从 placeholder 挪到输入条下面一行——发送失败时草稿不会清空，placeholder 被自己挡住，用户以前看不到任何原因。聊天窗口在输入框下面补一句「先排队，等连上会发出去」。
> - **第一次用给「下一步」**：`enabled` 的说明补上「打开后填好下面三项，点「立即连接」」，状态行下面固定写一句顺序（填值 → 立即连接 → 再去打开对方猫与聊天窗口，它们默认是关的）；缺地址或缺配对密码时「立即连接」直接置灰并写明缺哪一项；点「立即连接」顺手落盘地址时补一句「服务器地址已保存」（以前地址栏那条「改过了…」会无声消失）；连上对方时猫咪窗口给一句短暂的「已连上对方」（以前「连上了」只体现在偏好页一个小徽标里）。「最近一次错误」下面固定补一句人话（多半是地址填错 / 密码不对 / 服务器没开）。
> - **浮层里的附件气泡改成可点**：图片 / 语音 / 文件在浮层上只是一个 `[语音]` 这样的标签，点不动、也没说去哪儿看，而语音播放与附件操作都在默认关闭的聊天窗口里——现在点一下就打开聊天窗口（并把那条气泡的 mousedown 拦下来，否则会被窗口拖动吃掉点击）。
> - **语音发送失败会说「要重录」**：失败路径 Rust 已经删掉临时 wav，界面以前只留一句原始错误，用户会以为再点一次就能补发。现在拼一句 `voiceSendFailed`。发送中的那几百毫秒也不许再点试听（临时文件可能已被收走），试听键的 title 在播放中变成「停止试听」。
> - **待确认胶囊按连接状态分三句**（UX 复审 P2-B）：原来只分「联机没打开 / 对方不在线」两支，于是「正在连接」「还没连上服务器」「连不上服务器」也都写成「对方不在线，等对方回来再发」——用户会一直等对方，真实原因却可能是自己地址填错。`recordingBlockReason` 必须先按「能不能发」判「不用等」（判定落在 `recordingBlockReasonKey`，`stores/pair.ts`，单测在 `pair.spec.ts`），能发就返回空串、落到「录音 {seconds}s，待确认」：**`connected` 只说明自己连上了服务器，对方在不在线是 `peerOnline` 单独给的**（Rust 里 `connected` 与 `connected-peer-offline` 本来就是两个状态），只看连接状态会把「录好了、对方也在线」这个正常状态判成「对方不在线」（该 bug 由本轮复审抓出）。剩下按连接状态分：开关没开 → `sendRecordingDisabled`；`runtime.connection === 'peer-offline'` → `sendRecordingOffline`；其余（正在连 / 还没连上 / 连不上）→ `sendRecordingNotConnected`。正文与发送键的 title 同源。
> - **浮层会说明「更早的消息在聊天窗口里」**（UX 复审 P2-C）：浮层最多 3 条气泡，以前看着就像「聊天记录只有这几条」。新增 `pages.chat.hints.moreInChat`，与「发不出去的原因」**共用输入条下面那一行**（`statusNote = blockNote || moreNote`）：浮层是窄条，气泡上限 3 条就是按这个高度定的，再加一行会把最旧那条气泡挤出上沿。所以两件事不叠两行，能说的原因优先。
> - **设置页「连接状态」那一项改成 `vertical`**（UI 复审 P1）：R44 给它加了 `w-full` 的 Alert 与「下一步」说明，而 `ProListItem` 的默认插槽是**一行 flex**——标题块 `flex: 1 1 0%` 会被不换行的 `w-full` 兄弟压到 min-content（一个汉字一行的竖排字，和 R40 那张截图同一个毛病）。这个文件里其它 `w-full` 子节点都在 `vertical` 的项里，这两处是漏了。
> - **猫咪窗口的胶囊在窄窗口里不再被裁**（UI 复审 P2）：`recordingSkipped` / `recordingDiscarded` 补上 `max-w-full break-all text-center`（和 `recordingError` / `peerConnected` 同款）；两个操作胶囊加 `max-w-full`，正文那一段改成 `min-w-0 break-all text-center`（`min-w-0` 才允许它在 flex 里收缩换行，否则英文长句会顶出窗口右缘被 `overflow-hidden` 切掉）；三个图标补 `shrink-0`。
> - **胶囊上「试听 / 发送 / 取消」的可点范围放大**（UI 复审 P2）：图标仍是 `size-[1.2em]`，但加了一层透明的 `::before`（`relative before:content-empty before:absolute before:-inset-[0.5em]`）把命中区扩到约 2.2em——1.2em 在 300px 宽的窗口上只有 13px，而「取消」不可逆。**不能**改用 padding：`i-lucide:*` 是用 CSS mask（`mask-size: 100% 100%`）画的，加 padding 会把那层 mask 一起放大，图形跟着变大。（这条在 `dist` 的产物里核对过：`--ant-*` 那类失效的写法已经不剩，`::before` 的 `top/right/bottom/left:-.5em` 都在。）
> - **凭据库读失败不再被说成「还没有配置配对密码」**（UX 复审 P3）：`connectMissing` 依赖 `pairStore.hasSecret`，而 `hasSecret` 读失败时会停在 false——界面据此把「立即连接」永久置灰并说错原因。新增 `secretUnknown`（`pairHasSecret()` 失败时置 true，保存配对密码成功后置 false），这种情况下不下结论，让用户点下去看 Rust 的真实报错。
> - 文案面：`pushToTalk` 从「按住开始录音，松开发送」改成「松开后先让你确认再发送」（它和 R41 的实际行为一直不符）；对方上下线开关的标签与说明对齐（它只提示「回来」）；对方猫的输入统计补上单位；指纹那条改成「让对方念一下他那一串」（界面上拿不到对方的指纹，原来那句话做不到）；P2P 那行的「未启用」改成「未建立」（避免和联机总开关的「未启用」同词）；`sendRecordingDisabled` 统一成「双人联机没打开」（与 `pages.pairState.disabled` 同词）；`recordingDiscarded` 去掉「重新录了」那半句（什么都没录上时它不成立）；`p2p.hint` 的「打洞成功时」改成「若能直连」（用户读不懂「打洞」）；「对方猫 / 对方猫咪」统一成「对方猫」；省略号统一成「…」。
>
> **R45（两个密码明文显示 + 两处渲染修复，用户要求）** **R45 覆盖 R10 / R17 / R36 / R40 里「明文只写不读、保存后清空输入框」的表述**：
>
> - **配对密码与服务器密码和服务器地址一样一直显示在框里**：新增 `pair_get_secret` / `pair_get_server_password`（读回凭据库里的明文，没存过是 `None`）。设置页打开时把两个值填回框里，保存后不再清空（配对密码读回 Rust 规范化后的那串）；框里改过才能点「保存」，并提示「改过了，点保存才会生效」；「立即连接」只落盘改过的值，框被清空时显示回已保存值（和地址一栏同一处理）。值仍只存在系统凭据库，不进设置文件、Pinia 或日志。
> - **猫咪窗口开着聊天浮层时猫被往下推、底部被裁**：pixi 的 `resizeTo` 与 `live2d.resizeModel()` 一直按**整个窗口**算，而 R39 起猫只占浮层下面那一块。两者都改成按画布所在的那块区域（`canvas.parentElement`）；对方猫窗口与单机时这块区域就是整个窗口，行为不变。
> - **聊天窗口与浮层的原生控件没有样式**：这两个窗口没有 antd 组件，antd 的 `reset.css` 不清 `<button>` / `<textarea>` 的系统默认样式（白底、边框、黑字），于是底部那行提示成了白色方块、「打开 / 另存为」和播放键带系统边框、浮层气泡与输入的字是黑的。`global.scss` 里用元素选择器补一段重置（任何 class 都能盖过它），浮层根节点补 `text-[#fff]`。
>
> **R46（只发语音 + 浮层固定尺寸，用户要求）** **R46 覆盖 §37 / §38、R42 那句「现在只有粘贴图片与纸夹选文件两条路」，以及 §87 验收清单里「粘贴图片 / 选文件发送」各项的入口表述**（发送附件的验收只剩语音）：
>
> - **不能再发附件**：聊天窗口去掉回形针按钮与「粘贴图片即发送」，连同 `attaching` / `attachmentError` 两行提示、三条只给它们用的文案（`pickAttachment` / `sendingAttachment` / `attachmentBusy`），以及只为这两条路存在的前端函数（`pairSendAttachment` / `pairTransferPaths` / `pastedImageName` / `imageExtensionForMime` / `transferKindOfFile` 与对应单测）。能发的只有文字与语音（语音照旧从猫咪窗口的麦克风或「按住说话」快捷键发，走 `pair_send_recording`）。收附件、播放语音、另存为这些**接收侧**逻辑不动（语音本身走的就是附件管线，发送语音同样受「附件大小上限」约束）；Rust 的 `pair_send_attachment` / `pair_transfer_paths` 命令保留，前端已无入口。
> - **猫咪窗口的聊天浮层不随猫缩放**：气泡、输入条、状态行的字号 / 圆角 / 内边距 / 间距从 `vw` / `%` 改成固定像素（结构同聊天窗口的输入区，浮层取正文 12px、状态行 10px、发送键 22px）。浮层那一块的**高度**仍按 R39 的比例随猫缩放（约占窗口高 38%）；放不下的旧气泡从上沿裁掉。输入条 + 状态行约需 60px：窗口矮于约 160px 时状态行先隐藏（`@media (max-height: 160px)`），输入条约 42px 能完整显示到窗口约 110px 高；再小（缩放下限 10%）时输入条顶部会被裁，属于已知取舍。
>
> **平台事实（已核实，2026-09-23）**
>
...
**另有 R39 的第二块浮层**：猫咪窗口（`src/pages/main/index.vue` + `src/components/chat-overlay/index.vue`）
顶上那条气泡 + 输入条。两者读同一份聊天记录（Rust 侧 SQLite），各自显示各自的一屏：
聊天窗口可以翻历史、看附件卡片、复制/另存；猫咪窗口那条只显示最新几条与一条输入框。
