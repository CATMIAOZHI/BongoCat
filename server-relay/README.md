...
一套服务器同时承载多个双人会话。服务器**没有、也不需要**任何配对密码：它只做六件事：
...
6. 在 **UDP 3479** 上回答 STUN：告诉每个客户端「你的公网地址是多少」，P2P 直连全靠它（见「P2P 直连」一节）。
...
线上契约与 `../server-cloudflare/` 一致，差异是：`server.welcome` 里**可选**的 `limits` / `iceServers`（默认就会广告内置 STUN 的地址）、`/health` 里**可选**的 `"mode":"multi-pair"` 与 `"passwordRequired":true`、新增的 `X-Bongo-Room` / `X-Bongo-Server` 请求头、**HTTP 403**（服务器密码不对，CF 永远不会返回它），以及容量满时的 **HTTP 503**（CF 一个部署只服务一对用户，也永远不会返回它）。
...
| P2P 直连 | 只在两台电脑处于**同一个局域网**时成功（CF 不能跑 STUN） | 内置 STUN，不同家庭网络之间也能直连  |
| 适合     | 想省事                                                   | 想要自主、想上 60Hz、想要 P2P 直连   |

## 快速开始（推荐：域名 + 自动 HTTPS）

前提：一台有公网 IP 的机器（Linux，装了 Docker）、一个域名、能开 **TCP 80、TCP 443 与 UDP 3479**。
...
1. **云控制台的安全组放行 TCP 80、TCP 443 与 UDP 3479**（Caddy 要用 80 申请证书；direct 模式是 TCP 8080 + UDP 3479）。默认安全组往往只开了 22 / 3389。**UDP 3479 漏了不影响连接**，只是 P2P 那一行会一直显示「暂时连不上，已用服务器」。装了 coturn 的话还要放行 **3478/UDP + 3478/TCP、5349/TCP、49160~49260/UDP**；coturn 用的是 host 网络，Docker 的 iptables 规则**不会**替它放行，机器上开着 ufw / firewalld 时也要在那里放行（否则日志一切正常、打洞一直失败）。
...
客户端「服务器地址」填 `<公网 IP>:8080`（客户端会自动识别成 `ws://<IP>:8080/ws`），或者显式填 `http://<IP>:8080`。安全组放行 **TCP 8080 与 UDP 3479**（后者给 P2P 直连用）。
...
| `PAIR_STUN_PORT`             | `3479`         | 内置 STUN 的 UDP 端口（见「P2P 直连」），compose 的端口映射会跟着这个值走。`0` 关闭——**先**把 compose 里 `ports` 那行 udp 注释掉，否则 `0:0/udp` 会让 compose 起不来                                                                                                                                                                                                 |
| `PAIR_ICE_SERVERS`           | 无             | 可选，原样透传进 `server.welcome` 的 `iceServers`。**填了它就以它为准，内置 STUN 不启动**（见下）                                                                                                                                                                                                                                                                    |
...
P2P 直连能让猫咪状态和输入统计直接在两台电脑之间走、不经过服务器。打洞前，每台电脑得先问一台公网服务器「我的公网地址是多少」（这就是 STUN）。**问不到的话，两台电脑手里都只有局域网地址，隔着两个家庭网络一定连不上**——直连只剩「两台电脑在同一个局域网」这一种情况。

所以中继**自带**一个最小的 STUN 服务，默认开在 **UDP 3479**，你只需要：

1. 安全组放行 **UDP 3479**（compose 已经把这个端口发布出去了）；
2. 没有了。中继会用客户端连进来时的地址（`https://cat.example.com` 就是 `cat.example.com`，裸 IP 就是那个 IP）拼出 `stun:<地址>:3479`，在 `server.welcome` 里告诉**通过了服务器密码**的客户端。

自查：`docker compose logs relay` 开头应当有一行 `STUN      内置，UDP 3479`；客户端设置页的「P2P 通道」连上后显示「已直连」。一直显示「暂时连不上，已用服务器」时，先查 UDP 3479 有没有放行。

**已经部署过的旧版本**：拉新代码后 `docker compose up -d --build`（direct 模式加 `-f docker-compose.direct.yml`），再去安全组放行 UDP 3479。客户端不用改任何设置。

几点说明：

- **域名要直接指向这台机器的公网 IP**：客户端是按你填的服务器地址去问 STUN 的。域名套了 CDN / 代理（比如 Cloudflare 的橙色云朵）时，STUN 会打到 CDN 的节点上、拿不到回应；域名带 AAAA（IPv6）记录也可能先走 IPv6 而收不到（内置 STUN 只听 IPv4）。
- STUN 只是「告诉你公网地址」，**不转发任何数据**，所以几乎不占服务器带宽。你的公网 IP 只有你自己的这台服务器看得到（这也是它内置、而不是填一个公共 STUN 的原因）。
- 两个人**都**在运营商大内网（CGNAT）或对称型 NAT 后面时，有了 STUN 也可能打不通。那时客户端照样走服务器转发，功能不受影响；想再往前一步，就用下面的 TURN。
- 装了 **4.6 或更早版本**的 coturn 时，它会顺带占用 UDP 3479（`listening-port + 1`），和内置 STUN 撞端口；换一个 `PAIR_STUN_PORT` 即可（`coturn/coturn:latest` 没这个问题）。

## 可选：TURN（coturn）

两个人都在家庭 NAT / 校园网 / CGNAT 后面、内置 STUN 也打不通时，`coturn` 是成熟的开源兜底（TURN 会让数据经这台服务器中转，效果上和走中继差不多，一般不必装）：
...
起好之后把地址广告给客户端。**填了 `PAIR_ICE_SERVERS`，中继就以它为准、内置 STUN 不再启动**，所以这里要把 coturn 的 STUN 和 TURN 都写上（coturn 本身也提供 STUN）：
...
隐私提醒：STUN 必然让它所在的服务器看到双方的公网 IP，这是 WebRTC 的固有特性。中继默认只用**自己内置**的 STUN，从不填任何公共 STUN；你在 `PAIR_ICE_SERVERS` 里填了公共的，就会被客户端使用。
